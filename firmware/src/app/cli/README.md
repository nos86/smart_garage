...
- `pages.h` / `pages.cpp`
  ...
  - Exposes `kPages` with 6 tabs:
    ...
    - CAN (read-only MCP25625 transceiver diagnostics: link state from the boot-time loopback self-test, configured vs. register-computed bitrate, RX/TX frame counts and pkt/s rates, TEC/REC error counters and decoded EFLG error flags)
