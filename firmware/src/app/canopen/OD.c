/*******************************************************************************
    ...
    Originally generated by CANopenEditor v4.1-1-ga49a51a from CANopenNode's
    example/OD.c default DS301 profile, then copied here (this is our own
    project-owned file, not part of the vendored lib/CANopenNode submodule)
    and hand-adjusted for the smart_garage minimal CANopen bring-up:

      - x1017_producerHeartbeatTime: 0 (disabled) -> 1000 ms, since a working
        heartbeat producer is part of this bring-up's acceptance criteria.

    Everything else is the generator's default. Full custom Object
    Dictionary design (beyond this one field) is deferred -- see
    docs/architecture-plan.md's P0 backlog.
*******************************************************************************/
